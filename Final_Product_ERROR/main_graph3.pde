...
String detail_country ="None";
String detail_area = "None" ;
String detail_code = "aa";
float detail_value;
int[] detail_time = new int[6];

boolean isMouseinData(float x, float y,float radius){
  float distance = (mouseX-x)*(mouseX-x) + (mouseY-y)*(mouseY-y);
  if(distance <= radius){
    return true;
  }
  else return false;
  ...
  for(int year = start_year ; year <= end_year ; year++){
    for(TableRow row : NOAA_year[year-2000].rows()){
      ...
      if(isMouseinData(x,timelineY,value) && show_popup==-1){
        ...
        TableRow codeRow = CountryCoord.findRow(detail_country.charAt(0)+detail_country.substring(1).toLowerCase(),"name");
        detail_code = codeRow.getString("country"); // 첫번째 자료(파푸아뉴기니)에서 NullPointerException 발생함
        println(detail_code);
